...
static char *var_config_dir = NULL;

#ifdef WIN32
static char *config_utf16_to_utf8(wchar_t *unistr, long len, long *items_read,
                                  long *items_written) {
...
const char *config_get_config_dir() {
  ...
  if (__config_dir) return __config_dir;
 ...
 */
static char *config_generate_system_buid() { return config_generate_uuid(1); }

static int internal_set_value(const char *config_file, const char *key,
                              plist_t value) {
  ...
  if (plist_get_node_type(value) == PLIST_STRING) {
    ...
    usbmuxd_log(LL_DEBUG, "Setting key %s to %s in config file %s", key,
                value_string, config_file);
...
static int config_set_value(const char *key, plist_t value) {
  ...
  if (!result) {
    usbmuxd_log(LL_ERROR, "ERROR: Failed to write to '%s': %s", config_file,
                strerror(errno));
  }
...
static int internal_get_value(const char *config_file, const char *key,
                              plist_t *value) {
  *value = NULL;
  ...
  if (plist_read_from_filename(&config, config_file)) {
    usbmuxd_log(LL_DEBUG, "Reading key %s from config file %s", key,
                config_file);
    plist_t n = plist_dict_get_item(config, key);
    if (n) {
      *value = plist_copy(n);
...
static int config_set_system_buid(const char *system_buid) {
  return config_set_value(CONFIG_SYSTEM_BUID_KEY,
...
int config_has_device_record(const char *udid) {
  ...
  char *device_record_file =
      string_concat(config_path, DIR_SEP_S, udid, CONFIG_EXT, NULL);

  struct stat st;

  if ((stat(device_record_file, &st) == 0) && S_ISREG(st.st_mode)) res = 1;
 ...
 *
 * @note It is the responsibility of the calling function to free the returned
 * system_buid
 */
  ...
  if (!*system_buid) {
    ...
    *system_buid = config_generate_system_buid();
    if (!config_set_system_buid(*system_buid)) {
      usbmuxd_log(
          LL_WARNING,
          "WARNING: Failed to store SystemBUID, this might be a problem");
    }
  }
...
int config_set_device_record(const char *udid, char *record_data,
  ...
  if (!udid || !record_data || record_size < 8) return -EINVAL;
...
int config_get_device_record(const char *udid, char **record_data,
  ...
  if (!*record_data) {
    usbmuxd_log(LL_ERROR, "ERROR: Failed to read '%s': %s", device_record_file,
                strerror(errno));
    res = -ENOENT;
  }
  free(device_record_file);
...
int config_remove_device_record(const char *udid) {
  ...
  if (remove(device_record_file) != 0) {
    res = -errno;
    usbmuxd_log(LL_DEBUG, "Could not remove %s: %s", device_record_file,
                strerror(errno));
  }
...
static int config_device_record_get_value(const char *udid, const char *key,
                                          plist_t *value) {
  const char *config_path = NULL;
  char *config_file = NULL;
...
void config_device_record_get_host_id(const char *udid, char **host_id) {
  ...
  if (!*host_id) {
    usbmuxd_log(LL_ERROR,
                "ERROR: Could not get HostID from pairing record for udid %s",
                udid);
  }
}
