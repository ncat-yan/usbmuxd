...
enum client_state {
  ...
  CLIENT_CONNECTING2, // connection established, but waiting for response
                      // message to get sent
 ...
 * @return Same as recv() system call. Number of bytes read; when < 0 errno will
 * be set.
 */
int client_read(struct mux_client *client, void *buffer, uint32_t len) {
  usbmuxd_log(LL_SPEW, "client_read fd %d buf %p len %d", client->fd, buffer,
              len);
 ...
 * @return Same as system call send(). Number of bytes written; when < 0 errno
 * will be set.
 */
int client_write(struct mux_client *client, void *buffer, uint32_t len) {
  int sret = -1;

  usbmuxd_log(LL_SPEW, "client_write fd %d buf %p len %d", client->fd, buffer,
              len);
  if (client->state != CLIENT_CONNECTED) {
    usbmuxd_log(LL_ERROR,
  ...
  if (sret < 0) {
    if ((errno == EAGAIN) || (errno == EWOULDBLOCK)) {
      usbmuxd_log(LL_DEBUG, "client_write: fd %d not ready for writing",
                  client->fd);
      sret = 0;
    } else {
      usbmuxd_log(LL_ERROR, "ERROR: client_write: sending to fd %d failed: %s",
...
int client_accept(int listenfd) {
  ...
  if (log_level >= LL_INFO) {
    ...
    if (getpid() == cr.pid) {
      usbmuxd_log(LL_INFO, "Client %d accepted: %s[%d]", client->fd,
                  PACKAGE_NAME, cr.pid);
    } else {
...
void client_close(struct mux_client *client) {
  ...
  if (!found) {
    ...
    usbmuxd_log(LL_DEBUG, "%s: ignoring for non-existing client %p", __func__,
                client);
    mutex_unlock(&client_list_mutex);
    return;
  }
  ...
  if (log_level >= LL_INFO) {
    ...
    if (getpid() == cr.pid) {
      usbmuxd_log(LL_INFO, "Client %d is going to be disconnected: %s[%d]",
                  client->fd, PACKAGE_NAME, cr.pid);
    } else {
      char *process_name = _get_process_name_by_pid(cr.pid);
      usbmuxd_log(LL_INFO, "Client %d is going to be disconnected: %s[%d]",
                  client->fd, process_name, cr.pid);
      free(process_name);
    }
  }
#else
...
static int output_buffer_add_message(struct mux_client *client, uint32_t tag,
                                     enum usbmuxd_msgtype msg, void *payload,
                                     int payload_length) {
  ...
  usbmuxd_log(LL_DEBUG,
              "Client %d output buffer got tag %d msg %d payload_length %d",
              client->fd, tag, msg, payload_length);
  ...
  if (available < hdr.length) {
    ...
    uint32_t new_size =
        ((client->ob_capacity + hdr.length + 4096) / 4096) * 4096;
    usbmuxd_log(LL_DEBUG, "%s: Enlarging client %d output buffer %d -> %d",
                __func__, client->fd, client->ob_capacity, new_size);
  ...
  if (payload && payload_length)
    memcpy(client->ob_buf + client->ob_size + sizeof(hdr), payload,
           payload_length);
...
static int send_result(struct mux_client *client, uint32_t tag,
                       uint32_t result) {
  int res = -1;
  if (client->proto_version == 1) {
    ...
    /* binary packet */
    res = output_buffer_add_message(client, tag, MESSAGE_RESULT, &result,
                                    sizeof(uint32_t));
  }
  return res;
}

int client_notify_connect(struct mux_client *client,
                          enum usbmuxd_result result) {
  usbmuxd_log(LL_SPEW, "client_notify_connect fd %d result %d", client->fd,
              result);
...
static int send_pair_record(struct mux_client *client, uint32_t tag,
  ...
  if (record_data) {
    plist_t dict = plist_new_dict();
    plist_dict_set_item(dict, "PairRecordData",
                        plist_new_data(record_data, record_size));
    free(record_data);
    res = send_plist(client, tag, dict);
    plist_free(dict);
  } else {
...
static int send_device_add(struct mux_client *client, struct device_info *dev) {
  ...
  if (client->proto_version == 1) {
    ...
    res = output_buffer_add_message(client, 0, MESSAGE_DEVICE_ADD, &dmsg,
                                    sizeof(dmsg));
  }
  return res;
}
...
static int send_device_paired(struct mux_client *client, uint32_t device_id) {
  ...
  if (client->proto_version == 1) {
    ...
    /* binary packet */
    res = output_buffer_add_message(client, 0, MESSAGE_DEVICE_PAIRED,
                                    &device_id, sizeof(uint32_t));
  }
  return res;
}
...
static int handle_command(struct mux_client *client,
                          struct usbmuxd_header *hdr) {
  int res;
  usbmuxd_log(LL_DEBUG, "Client %d command len %d ver %d msg %d tag %d",
              client->fd, hdr->length, hdr->version, hdr->message, hdr->tag);

  if (client->state != CLIENT_COMMAND) {
    usbmuxd_log(
        LL_ERROR,
        "Client %d command received in the wrong state, got %d but want %d",
        client->fd, client->state, CLIENT_COMMAND);
  ...
  if ((hdr->version != 0) && (hdr->version != 1)) {
    usbmuxd_log(LL_INFO, "Client %d version mismatch: expected 0 or 1, got %d",
                client->fd, hdr->version);
  ...
  case MESSAGE_PLIST:
    ...
    if (!dict) {
      ...
      if (!strcmp(message, "Listen")) {
        ...
        usbmuxd_log(LL_DEBUG,
                    "Client %d requesting connection to device %d port %d",
                    client->fd, device_id, ntohs(portnum));
          ...
          if (res < 0) {
            ...
            if (dev_id > 0) {
              ...
              for (i = 0; devs && i < count; i++, dev++) {
                if ((uint32_t)dev->id == dev_id &&
                    (strcmp(dev->serial, record_id) == 0)) {
                  found++;
                  break;
                }
              }
              if (!found) {
                usbmuxd_log(LL_ERROR,
                            "ERROR: SavePairRecord: DeviceID %d (%s) is not "
                            "connected\n",
                            dev_id, record_id);
              } else {
  ...
  case MESSAGE_CONNECT:
    ch = (void *)hdr;
    usbmuxd_log(LL_DEBUG, "Client %d connection request to device %d port %d",
                client->fd, ch->device_id, ntohs(ch->port));
    ...
    return 0;
  default:
    usbmuxd_log(LL_ERROR, "Client %d invalid command %d", client->fd,
                hdr->message);
    if (send_result(client, hdr->tag, RESULT_BADCOMMAND) < 0)
      return -1;
    return 0;
...
static void output_buffer_process(struct mux_client *client) {
  int res;
  if (!client->ob_size) {
    usbmuxd_log(LL_WARNING, "Client %d OUT process but nothing to send?",
                client->fd);
    client->events &= ~POLLOUT;
    return;
  }
  ...
  if (res <= 0) {
    usbmuxd_log(LL_ERROR, "Sending to client fd %d failed: %d %s", client->fd,
                res, strerror(errno));
    client_close(client);
    return;
  }
  if ((uint32_t)res == client->ob_size) {
    ...
    if (client->state == CLIENT_CONNECTING2) {
      usbmuxd_log(LL_DEBUG, "Client %d switching to CONNECTED state",
                  client->fd);
...
static void input_buffer_process(struct mux_client *client) {
  ...
  if (client->ib_size < sizeof(struct usbmuxd_header)) {
    res = recv(client->fd, client->ib_buf + client->ib_size,
               sizeof(struct usbmuxd_header) - client->ib_size, 0);
    if (res <= 0) {
      if (res < 0)
        usbmuxd_log(LL_ERROR, "Receive from client fd %d failed: %s",
                    client->fd, strerror(errno));
      else
  ...
  if (hdr->length > client->ib_capacity) {
    usbmuxd_log(LL_INFO, "Client %d message is too long (%d bytes)", client->fd,
                hdr->length);
    client_close(client);
    return;
  }
  if (hdr->length < sizeof(struct usbmuxd_header)) {
    usbmuxd_log(LL_ERROR, "Client %d message is too short (%d bytes)",
                client->fd, hdr->length);
    client_close(client);
    return;
  }
  if (client->ib_size < hdr->length) {
    ...
    res = recv(client->fd, client->ib_buf + client->ib_size,
               hdr->length - client->ib_size, 0);
    if (res < 0) {
      usbmuxd_log(LL_ERROR, "Receive from client fd %d failed: %s", client->fd,
                  strerror(errno));
      client_close(client);
      return;
    } else if (res == 0) {
...
void client_process(int fd, short events) {
  ...
  if (client->state == CLIENT_CONNECTED) {
    ...
    } else if (events & POLLOUT) { // not both in case client died as part of
                                   // process_recv
...
void client_device_add(struct device_info *dev) {
  mutex_lock(&client_list_mutex);
  usbmuxd_log(LL_DEBUG, "client_device_add: id %d, location 0x%x, serial %s",
              dev->id, dev->location, dev->serial);
...
void client_shutdown(void) {
  ...
  FOREACH(struct mux_client * client, &client_list) { client_close(client); }
  ENDFOREACH
